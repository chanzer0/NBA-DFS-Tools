...
class NBA_Showdown_Optimizer:
    ...
    def __init__(self, site=None, num_lineups=0, num_uniques=1):
        ...
        self.problem = plp.LpProblem(f"NBA-{site}", plp.LpMaximize)
                ...
                if self.site == "dk":
                    ...
                    for pos in ["MVP", "STAR", "PRO", "UTIL"]:
                        if (player_name, pos, team) in self.player_dict:
                            self.player_dict[(player_name, pos, team)]["ID"] = row[
                                "Id"
                            ].replace("-", "#")
                            self.player_dict[(player_name, pos, team)]["Matchup"] = row[
                                "Game"
                            ]
    ...
    def optimize(self):
        ...
        for player, attributes in self.player_dict.items():
            ...
            lp_variables[(player, player[1], player_id)] = plp.LpVariable(
                name=f"{player}_{player[1]}_{player_id}", cat=plp.LpBinary
            )
        ...
        if self.site == "dk":
            # 1 CPT
            cpt_players = [
                (player, "CPT", attributes["ID"])
                for player, attributes in self.player_dict.items()
                if player[1] == "CPT"
            ]
            self.problem += (
                plp.lpSum(lp_variables[player] for player in cpt_players) == 1,
                "Must have 1 CPT",
            )

            # 5 UTIL
            util_players = [
                (player, "UTIL", attributes["ID"])
                for player, attributes in self.player_dict.items()
                if player[1] == "UTIL"
            ]
            self.problem += (
                plp.lpSum(lp_variables[player] for player in util_players) == 5,
                "Must have 5 UTIL",
            )
            ...
            for player_key_list in player_key_groups:
                self.problem += (
                    plp.lpSum(
                        lp_variables[(pk, pk[1], self.player_dict[pk]["ID"])]
                        for pk in player_key_list
                    )
                    <= 1,
            ...
            # 1 MVP
            mvp_players = [
                (player, "MVP", attributes["ID"])
                for player, attributes in self.player_dict.items()
                if player[1] == "MVP"
            ]
            self.problem += (
                plp.lpSum(lp_variables[player] for player in mvp_players) == 1,
                "Must have 1 MVP",
            )

            # 1 STAR
            star_players = [
                (player, "STAR", attributes["ID"])
                for player, attributes in self.player_dict.items()
                if player[1] == "STAR"
            ]
            self.problem += (
                plp.lpSum(lp_variables[player] for player in star_players) == 1,
                "Must have 1 STAR",
            )

            # 1 PRO
            pro_players = [
                (player, "PRO", attributes["ID"])
                for player, attributes in self.player_dict.items()
                if player[1] == "PRO"
            ]
            self.problem += (
                plp.lpSum(lp_variables[player] for player in pro_players) == 1,
                "Must have 1 PRO",
            )
            ...
            self.problem += (
                plp.lpSum(lp_variables[player] for player in util_players) == 2,
                "Must have 2 UTIL",
            )
        ...
        for i in range(self.num_lineups):
            ...
            try:
                ...
                print(
                    "[ERROR] Infeasibility reached - only generated {} lineups out of {}. Continuing with export.".format(
    ...
    def output(self):
        ...
        out_path = os.path.join(
            os.path.dirname(__file__),
            "../output/{}_sd_optimal_lineups_{}.csv".format(
